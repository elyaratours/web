...
import { getConfiguredSite } from '@/lib/seo';
...
function url(path: string, site: URL) {
...
function entry(item: SitemapEntry, site: URL) {
  ...
  const alternates = item.alternates && item.alternates.length > 1
    ...
    <xhtml:link rel="alternate" hreflang="${hreflang(alternate.locale)}" href="${escapeXml(url(alternate.path, site))}" />`).join('')
      + `
    <xhtml:link rel="alternate" hreflang="x-default" href="${escapeXml(url(item.alternates.find((alternate) => alternate.locale === 'es')?.path ?? item.path, site))}" />`
    : '';

  return `  <url>
    <loc>${escapeXml(url(item.path, site))}</loc>${lastmod}${changefreq}${priority}${alternates}
  </url>`;
}
...
export async function GET({ site: astroSite }: { site?: URL }) {
  const site = getConfiguredSite(astroSite);
...
${uniqueEntries.map((item) => entry(item, site)).join('\n')}
</urlset>`;
