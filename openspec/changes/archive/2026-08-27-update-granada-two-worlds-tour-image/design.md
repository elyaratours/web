...
The site is a static Astro tourism website with localized, data-driven tour entries. Tour images are configured in Markdown frontmatter through the `image` field and rendered by shared catalog/detail templates. The requested image exists at `public/images/centro-albayzin.jpg` and fits the content schema when referenced as `/images/centro-albayzin.jpg`.

## Goals / Non-Goals

**Goals:**
- Use `/images/centro-albayzin.jpg` for the Spanish `Granada entre dos mundos` tour.
- Use `/images/centro-albayzin.jpg` for the English `Granada Between Two Worlds` tour.
...
- Reference the existing JPEG directly.
  Alternative considered: convert the asset to WebP or AVIF. Rejected for this change because the user specifically requested the existing `public/images/centro-albayzin.jpg` file and no asset optimization was required.

- Update only the two requested localized entries.
  Alternative considered: update all Granada historical-route entries. Rejected because the requested scope names exactly two public tour titles.
